-- ==============================================================================
-- ShopSphere Database Schema & Seed Data (Stage 5 - SQS + AWS Lambda + RDS)
-- Safe, idempotent execution for concurrent ASG & Lambda bootstrapping
-- ==============================================================================
...
CREATE TABLE IF NOT EXISTS orders (
    ...
    processed_by_worker VARCHAR(150),
...
-- Ensure upgrade columns exist if migrated from earlier stages
DO $$
BEGIN
    IF NOT EXISTS (
    ...
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name = 'orders' AND column_name = 'processed_by_worker'
    ) THEN
        ALTER TABLE orders ADD COLUMN processed_by_worker VARCHAR(150);
    END IF;
