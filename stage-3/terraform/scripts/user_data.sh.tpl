#!/bin/bash
# ==============================================================================
# ShopSphere Stage 3 Bootstrap Script (ALB + Auto Scaling Group + Amazon RDS)
...
echo "=== ShopSphere Stage 3 ASG Node Bootstrap Starting at $(date) ==="
...
# ------------------------------------------------------------------------------
echo "[3/6] Setting up ShopSphere Stage 3 application source code..."
...
[Unit]
Description=ShopSphere Application Server (Stage 3 - ALB + ASG + Multi-EC2)
...
server {
    ...
    # Pass client and proxy tracking headers
    location / {
...
for i in $(seq 1 30); do
  if curl -sf "http://127.0.0.1:${app_port}/health" > /dev/null 2>&1; then
    echo "✅ ShopSphere Stage 3 node is UP and healthy!"
    break
  fi
...
echo "=== ShopSphere Stage 3 Node Bootstrap Completed at $(date) ==="
echo "Node ready to serve ALB Target Group traffic."
