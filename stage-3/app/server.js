...
// ==============================================================================
// PostgreSQL Connection Pool (Amazon RDS PostgreSQL)
// ==============================================================================
const poolConfig = {
...
// Add custom response header indicating serving host (useful for ALB debugging)
...
app.get('/health', async (req, res) => {
  ...
  res.status(isHealthy ? 200 : 503).json({
    ...
    application: 'ShopSphere Scaled Application Tier',
    version: '3.0.0',
...
app.get('/api/system/info', (req, res) => {
  res.json({
    ...
    version: '3.0.0',
    stage: 'Stage 3 - ALB + Auto Scaling Group + Multi-EC2 + Amazon RDS',
    architecture: 'Internet -> ALB (:80) -> Target Group -> Auto Scaling Group (EC2 Instances in AZ-A & AZ-B) -> Amazon RDS PostgreSQL (:5432)',
    hostname: os.hostname(),
...
app.get('/api/products/:id', async (req, res) => {
  try {
    ...
    res.json({ success: true, data: rows[0], servedBy: os.hostname() });
  } catch (err) {
...
app.post('/api/orders', async (req, res) => {
  ...
  try {
    ...
    // Insert main order record with current serving EC2 hostname
    ...
    res.status(201).json({
      ...
      message: 'Order created successfully with Amazon RDS transactional consistency'
    });
  } catch (err) {
...
app.listen(PORT, async () => {
  console.log(`=======================================================`);
  console.log(`🚀 ShopSphere Stage 3 Server running on port ${PORT}`);
  console.log(`🌐 Serving Instance Hostname: ${os.hostname()}`);
  console.log(`🔗 Database Target: ${poolConfig.host}:${poolConfig.port}`);
  console.log(`=======================================================`);
