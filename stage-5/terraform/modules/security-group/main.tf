# ==============================================================================
# Security Group Module - Stage 4 Multi-Tier Firewalls
# ALB -> EC2 ASG Fleet -> Amazon RDS (5432) & Amazon ElastiCache Redis (6379)
# ==============================================================================
...
resource "aws_security_group" "ec2" {
  ...
  # Outbound - Allow all egress (package downloads, git cloning, RDS & Redis connectivity)
  egress {
    description      = "Allow all outbound traffic"
    from_port        = 0
    to_port          = 0
...
# 4. Amazon ElastiCache Redis Security Group (In-Memory Cache Tier)
resource "aws_security_group" "elasticache" {
  name        = "${var.project_name}-${var.environment}-redis-sg"
  description = "Security group for ShopSphere Amazon ElastiCache Redis cluster"
  vpc_id      = var.vpc_id

  # Redis - Strictly restricted to EC2 Security Group ONLY
  ingress {
    description     = "Allow Redis access strictly from EC2 application tier"
