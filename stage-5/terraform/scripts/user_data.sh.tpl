#!/bin/bash
# ==============================================================================
# ShopSphere Stage 5 Bootstrap Script (SQS + Lambda + ElastiCache + RDS)
...
echo "=== ShopSphere Stage 5 ASG Node Bootstrap Starting at $(date) ==="
...
# ------------------------------------------------------------------------------
echo "[3/6] Setting up ShopSphere Stage 5 application source code..."
...
if [ -d "/opt/shopsphere/repo/stage-5/app" ]; then
...
[Unit]
Description=ShopSphere Application Server (Stage 5 - SQS + Lambda + ElastiCache + RDS)
...
for i in $(seq 1 30); do
  if curl -sf "http://127.0.0.1:${app_port}/health" > /dev/null 2>&1; then
    echo "✅ ShopSphere Stage 5 node is UP and healthy!"
    break
  fi
...
echo "=== ShopSphere Stage 5 Node Bootstrap Completed at $(date) ==="
