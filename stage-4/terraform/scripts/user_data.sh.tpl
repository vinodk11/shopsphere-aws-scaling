#!/bin/bash
# ==============================================================================
# ShopSphere Stage 4 Bootstrap Script (ALB + ASG + ElastiCache Redis + RDS)
...
echo "=== ShopSphere Stage 4 ASG Node Bootstrap Starting at $(date) ==="
...
# ------------------------------------------------------------------------------
echo "[3/6] Setting up ShopSphere Stage 4 application source code..."
...
if [ -d "/opt/shopsphere/repo/stage-4/app" ]; then
  echo "Deploying application code from stage-4/app..."
  cp -r /opt/shopsphere/repo/stage-4/app/* /opt/shopsphere/app/
elif [ -d "/opt/shopsphere/repo/stage-3/app" ]; then
  echo "Deploying application code from stage-3/app..."
  cp -r /opt/shopsphere/repo/stage-3/app/* /opt/shopsphere/app/
...
# ------------------------------------------------------------------------------
echo "[4/6] Writing .env configuration pointing to Amazon RDS & Amazon ElastiCache..."
...
[Unit]
Description=ShopSphere Application Server (Stage 4 - ElastiCache Redis + RDS)
...
for i in $(seq 1 30); do
  if curl -sf "http://127.0.0.1:${app_port}/health" > /dev/null 2>&1; then
    echo "✅ ShopSphere Stage 4 node is UP and healthy!"
    break
  fi
...
echo "=== ShopSphere Stage 4 Node Bootstrap Completed at $(date) ==="
echo "Connected to RDS (${db_host}) and ElastiCache Redis (${redis_host})"
