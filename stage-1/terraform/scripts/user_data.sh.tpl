#!/usr/bin/env bash
# ==============================================================================
# ShopSphere EC2 Bootstrap Script (Stage 1)
# Provisions: Node.js runtime, PostgreSQL 15, Nginx reverse proxy, ShopSphere App
# ==============================================================================
...
echo "===================================================================="
echo "[ShopSphere Bootstrap] Starting Stage 1 Single EC2 Provisioning..."
...
# ------------------------------------------------------------------------------
echo "[1/7] Updating OS and installing dependencies (Node.js, PostgreSQL, Nginx, Git)..."
dnf update -y
dnf install -y git nodejs npm postgresql15-server postgresql15 nginx

# ------------------------------------------------------------------------------
# 2. Configure & Initialize PostgreSQL 15 Database
# ------------------------------------------------------------------------------
echo "[2/7] Initializing PostgreSQL 15 database..."
...
echo "[2/7] Starting and enabling PostgreSQL service..."
systemctl daemon-reload
systemctl enable --now postgresql

echo "[2/7] Provisioning database '${db_name}' and user '${db_user}'..."
...
# Grant schema permissions in target database for PostgreSQL 15+
...
# ------------------------------------------------------------------------------
echo "[3/7] Setting up ShopSphere application directory..."
mkdir -p /opt/shopsphere/app
mkdir -p /opt/shopsphere/repo

APP_REPO="${app_repo_url}"
if [ -n "$APP_REPO" ]; then
  echo "[3/7] Cloning ShopSphere application repository from $APP_REPO..."
  if git clone "$APP_REPO" /opt/shopsphere/repo; then
    echo "[3/7] Cloned successfully into /opt/shopsphere/repo."
    if [ -d "/opt/shopsphere/repo/stage-1/app" ]; then
      echo "[3/7] Copying application code from stage-1/app..."
      cp -r /opt/shopsphere/repo/stage-1/app/* /opt/shopsphere/app/
    elif [ -d "/opt/shopsphere/repo/shopsphere/stage-1/app" ]; then
      echo "[3/7] Copying application code from shopsphere/stage-1/app..."
      cp -r /opt/shopsphere/repo/shopsphere/stage-1/app/* /opt/shopsphere/app/
    elif [ -d "/opt/shopsphere/repo/app" ]; then
      echo "[3/7] Copying application code from app/..."
      cp -r /opt/shopsphere/repo/app/* /opt/shopsphere/app/
    fi
  else
    echo "[3/7] Git clone fallback: using self-contained Stage 1 application files..."
  fi
fi

# Ensure package.json exists (fallback / embedded monolith code)
...
{
  "name": "shopsphere-monolith",
  "version": "1.0.0",
  "description": "ShopSphere E-Commerce Monolithic Application - Stage 1",
  "main": "server.js",
  "scripts": {
...
(1, 'Audio & Electronics', 'High fidelity audio equipment and accessories'),
(2, 'Wearables', 'Next-gen smart wearables and health trackers'),
(3, 'Smart Home', 'Connected devices for modern automation'),
(4, 'Computer Peripherals', 'Productivity tools and mechanical gear')
...
(3, 1, 'UltraHD 4K Action Camera', 'Compact waterproof 4K/60fps camera with 6-axis gyro stabilization.', 129.99, 25, 'https://images.unsplash.com/photo-1526170375885-4d8ecf77b99f?w=500&auto=format&fit=crop&q=60'),
(4, 3, 'Aurora Smart RGB Desk Lamp', 'Voice-controlled ambient lighting with custom gradients and adaptive brightness.', 49.99, 60, 'https://images.unsplash.com/photo-1507473885765-e6ed057f782c?w=500&auto=format&fit=crop&q=60'),
(5, 4, 'CyberDeck RGB Mechanical Keyboard', 'Hot-swappable mechanical switches, PBT keycaps, and aircraft-grade aluminum frame.', 89.99, 40, 'https://images.unsplash.com/photo-1587829741301-dc798b83add3?w=500&auto=format&fit=crop&q=60'),
(6, 1, 'HyperCharge 20000mAh Power Bank', '65W USB-C Power Delivery fast-charging power bank for laptops and phones.', 39.99, 75, 'https://images.unsplash.com/photo-1609592426507-da665123d537?w=500&auto=format&fit=crop&q=60')
ON CONFLICT (id) DO NOTHING;
...
async function initializeDatabase() {
  ...
  if (fs.existsSync(schemaPath)) {
    try {
      ...
      console.error('[DB] Schema initialization warning:', err.message);
    }
  }
}
...
app.get('/health', async (req, res) => {
  ...
  res.status(isHealthy ? 200 : 503).json({
    ...
    stage: 'Stage 1: Single EC2 + Local Database',
...
<head>
  ...
  <title>ShopSphere - Modern E-Commerce Monolith</title>
...
<body>
  <div class="stage-banner">
    <div class="container banner-content">
      <span class="badge">AWS Stage 1</span>
      <span class="stage-desc">Single EC2 Instance (Amazon Linux 2023) + Colocated PostgreSQL Database + Nginx Reverse Proxy</span>
  ...
  <header class="navbar">
    <div class="container nav-container">
      <div class="brand">
        ...
        <div>
          <h1>ShopSphere</h1>
          <span class="tagline">Cloud Evolution Architecture</span>
        </div>
      </div>
  ...
  <section class="hero">
    <div class="container">
      ...
      <p>Demonstrating cloud-native application evolution starting with a resilient single-server baseline.</p>
    </div>
  </section>
  ...
  <div id="infoModal" class="modal">
    <div class="modal-content">
      <div class="modal-header">
        <h3>Architecture Details (Stage 1)</h3>
        <button class="close-btn" id="closeInfoModal">&times;</button>
      </div>
      <div class="modal-body">
        <div class="arch-box">
          <code>Internet &rarr; Nginx (:80) &rarr; Node.js Express (:8080) &rarr; PostgreSQL (:5432) [Localhost]</code>
        </div>
        <div class="metrics-grid" id="systemMetrics">
          ...
          <div class="metric-card"><span class="metric-label">Database</span><span class="metric-val" id="metricDb">-</span></div>
        ...
        <h4 style="margin-top: 1.5rem; margin-bottom: 0.5rem;">Recent Orders in DB</h4>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <form id="checkoutForm" class="checkout-form" style="display: none;">
          ...
          <button type="submit" id="submitOrderBtn" class="btn btn-primary btn-block">Confirm & Place Order</button>
        </form>
      </div>
    </div>
  </div>
  <footer class="footer">
    <div class="container footer-content">
      ...
      <p class="footer-sub">Stage 1: Single EC2 + Local PostgreSQL &bull; Managed by Terraform</p>
    </div>
  </footer>
  <script>
    let cart = [], products = [];
    ...
    async function checkHealth() {
      try {
        ...
        if (data.status === 'UP') {
          ...
          badge.innerHTML = '<span class="pulse green"></span> System Healthy (DB: ' + data.database.latencyMs + 'ms)';
        } else {
          badge.className = 'health-badge health-down';
          badge.innerHTML = '<span class="pulse red"></span> DB Issue';
        }
      } catch (e) {
        const badge = document.getElementById('systemHealthBadge');
        badge.className = 'health-badge health-down';
    ...
    document.getElementById('checkoutForm').addEventListener('submit', async (e) => {
      ...
      finally { btn.disabled = false; btn.innerText = 'Confirm & Place Order'; }
    });
    ...
    document.getElementById('systemInfoBtn').onclick = async () => {
      ...
      try {
        ...
        document.getElementById('metricDb').innerText = hData.database.status + ' (' + (hData.database.latencyMs || 0) + 'ms)';
...
.stage-banner { background: linear-gradient(90deg, #1e1b4b 0%, #312e81 100%); border-bottom: 1px solid #4338ca; padding: 0.6rem 0; font-size: 0.85rem; }
.banner-content { display: flex; align-items: center; justify-content: space-between; gap: 1rem; flex-wrap: wrap; }
.badge { background: #6366f1; color: #fff; font-weight: 700; padding: 0.2rem 0.6rem; border-radius: 6px; text-transform: uppercase; font-size: 0.75rem; letter-spacing: 0.05em; }
.stage-desc { color: #c7d2fe; flex: 1; }
...
.hero { padding: 3rem 0 2rem; text-align: center; background: radial-gradient(circle at top, rgba(79, 70, 229, 0.15) 0%, transparent 70%); }
...
.modal { display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.7); backdrop-filter: blur(4px); z-index: 100; overflow-y: auto; }
.modal-content { background: var(--bg-card); max-width: 540px; margin: 5vh auto; border-radius: var(--radius); border: 1px solid var(--border-color); box-shadow: var(--shadow); overflow: hidden; }
...
.arch-box { background: #0f172a; padding: 0.75rem; border-radius: 6px; border: 1px solid #334155; font-size: 0.8rem; font-family: 'JetBrains Mono', monospace; margin-bottom: 1rem; color: #38bdf8; word-break: break-all; }
...
# ------------------------------------------------------------------------------
# 4. Write Application Environment File & Install Node Packages
# ------------------------------------------------------------------------------
...
echo "[4/7] Installing application dependencies via npm..."
...
# ------------------------------------------------------------------------------
echo "[5/7] Configuring systemd service for ShopSphere..."
...
# ------------------------------------------------------------------------------
# 6. Configure Nginx as Reverse Proxy (Port 80 -> Application Port)
# ------------------------------------------------------------------------------
...
# Remove or comment default server blocks if conflicting
...
# ------------------------------------------------------------------------------
echo "[7/7] Verifying ShopSphere service health..."
sleep 5
for i in {1..10}; do
  ...
  echo "Waiting for ShopSphere service to respond... ($i/10)"
  sleep 3
done

echo "===================================================================="
echo "[ShopSphere Bootstrap] Stage 1 EC2 setup completed successfully!"
echo "Timestamp: $(date -u '+%Y-%m-%d %H:%M:%SZ')"
echo "===================================================================="
