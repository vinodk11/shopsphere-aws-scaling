...
variable "instance_class" {
  description = "RDS DB instance class (e.g. db.t3.micro, db.t4g.micro)"
  type        = string
  default     = "db.t3.micro"
}
...
variable "storage_type" {
  description = "Storage type for the RDS instance (e.g. gp3, gp2)"
  type        = string
  default     = "gp3"
}

variable "multi_az" {
  description = "Specifies if the RDS instance is deployed across multiple AZs for high availability"
  type        = bool
  default     = false
}

variable "publicly_accessible" {
  description = "Whether the database is publicly accessible over the internet (should be false for security)"
  type        = bool
  default     = false
}
